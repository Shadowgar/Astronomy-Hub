...
static bool properties_get_bool(const json_value *props, const char *key,
                                bool default_value)
{
    const char *str = json_get_attr_s(props, key);
    if (!str) return default_value;
    if (!strcasecmp(str, "true") || !strcasecmp(str, "yes") ||
        !strcasecmp(str, "on") || !strcmp(str, "1"))
        return true;
    if (!strcasecmp(str, "false") || !strcasecmp(str, "no") ||
        !strcasecmp(str, "off") || !strcmp(str, "0"))
        return false;
...
static int stars_add_data_source(obj_t *obj, const char *url, const char *key)
{
    ...
    survey->show_labels =
        properties_get_bool(args, "oras_show_labels", true);
