...
<template>
  <v-card v-if="selectedObject" transparent style="background: rgba(66, 66, 66, 0.3);">
    ...
    <v-card-text v-if="orasMetadata" class="oras-enhanced-panel">
      <v-chip x-small color="cyan darken-3" text-color="white" class="mb-2">ORAS Enhanced</v-chip>
      <v-row no-gutters>
        <v-col cols="4">Catalog IDs</v-col>
        <v-col cols="8" class="white--text">{{ orasMetadata.catalogIds.join(', ') }}</v-col>
      </v-row>
      <v-row no-gutters>
        <v-col cols="4">Source attribution</v-col>
        <v-col cols="8" class="white--text">{{ orasMetadata.sources.join(', ') }}</v-col>
      </v-row>
      <v-row v-if="orasMetadata.pack" no-gutters>
        <v-col cols="4">Catalog pack</v-col>
        <v-col cols="8" class="white--text">{{ orasMetadata.pack }}</v-col>
      </v-row>
      <v-row v-for="property in orasProperties" :key="property.key" no-gutters>
        <v-col cols="4">{{ property.key }}</v-col>
        <v-col cols="8" class="white--text">{{ property.value }}</v-col>
      </v-row>
      <div v-if="orasProperties.length === 0" class="grey--text text-caption mt-2">
        Physical properties: Unavailable from mounted sources
      </div>
    </v-card-text>
    <v-card-text>
...
export default {
  ...
  computed: {
    ...
    orasMetadata: function () {
      ...
      const sources = (source.source_attribution || [])
    ...
    orasProperties: function () {
      ...
      const fields = [
        ...
        ['Radial velocity', 'radial_velocity_km_s'],
      ...
      if (this.selectedObject.double_star) {
        ...
        if (double.position_angle_deg != null) properties.push({ key: 'Position angle', value: double.position_angle_deg + ' deg' })
      }
      return properties
    },
...
.oras-enhanced-panel {
  border-top: 1px solid rgba(128, 222, 234, 0.35);
  border-bottom: 1px solid rgba(128, 222, 234, 0.2);
  background: rgba(0, 96, 100, 0.16);
}
</style>
