...
describe('Module 2 deterministic port replay (G4)', () => {
  ...
  it('includes deterministic slices for traversal, HIP lookup, and catalog astrometry seams', () => {
    ...
    expect(fingerprint).toContain('stars-c-module-runtime|')
    expect(fingerprint).toContain('stars-c-frame-pacing|')
    expect(fingerprint).toContain('stars-c-label-runtime|')
  })
})
