...
describe('sky core runtime boundary', () => {
  ...
  it('registers explicit runtime modules for atmosphere, background, stars, objects, and overlays', () => {
    const sceneSource = fs.readFileSync(SKY_ENGINE_SCENE_PATH, 'utf8')
    const bridgeSource = fs.readFileSync(SKY_RUNTIME_BRIDGE_PATH, 'utf8')
    ...
    expect(atmosphereModuleSource).toContain('prepareDirectBackgroundFrame(')
    expect(backgroundModuleSource).toContain('drawSyntheticDensityStars(')
    expect(backgroundModuleSource).not.toContain('prepareDirectBackgroundFrame(')
  ...
  it('registers runtime modules in explicit render order and keeps the bridge last', () => {
    ...
    expect(backgroundRegistrationIndex).toBeGreaterThan(atmosphereRegistrationIndex)
    ...
    expect(backgroundModuleSource).toContain('renderOrder: 15')
