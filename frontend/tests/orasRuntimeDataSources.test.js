...
describe('oras runtime data sources', () => {
  ...
  it('validates exact deep links, satellite parsing, and known DSO detail types', () => {
    ...
    expect(source).toContain("const defaultVisibleSatelliteTestTime = '2026-06-04T00:00:00Z'")
    ...
    expect(source).toContain('const materializedModelData = selectedObject.model_data || {}')
    expect(source).toContain('materializedModelData.ra')
    expect(source).toContain('materializedModelData.de')
    expect(source).toContain("selection.getInfo('radec')")
