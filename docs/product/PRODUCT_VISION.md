...
This document defines the full vision, system identity, and long-term capabilities of Astronomy Hub.
...
* drive implementation steps
* act as task authority
...
* feature validation against vision
* long-term alignment
...
## 10. 🧩 INTERFACE & INTERACTION MODEL (LOCKED)

---

### 10.1 Command Center Layout

Astronomy Hub is presented as a **command center interface**.

The layout consists of:

* a central live viewport
* surrounding contextual panels
* feed-driven interaction

The system is not a page-based UI.
It is a **persistent interactive environment**.

---

### 10.2 Active Engine Viewport (CRITICAL RULE)

The center of the interface is always:

> **The Active Engine Viewport**

At all times:

```text
Center Viewport = Active Engine
```

This viewport is:

* a live rendered scene
* controlled by the currently active engine
...
* provide domain-specific exploration

---

### 10.4 Feed-Driven Interaction Model

All interaction originates from either:

* the viewport (direct object click)
* surrounding feeds (context-driven)
...
```text
User clicks feed item
→ system determines owning engine
→ active engine may change
→ viewport updates
→ object is focused
→ detail is opened
```
...
### 10.5 Object Interaction Model

Every object in the system:

* is owned by an engine
* is clickable
* opens a detail layer
* provides deeper navigation
...
### 10.6 No Dead-End Rule (MANDATORY)

The system must never present a terminal view.
...
### 10.7 Scene Instancing (UI Enforcement)

The interface enforces scene instancing:

* only one engine scene is active at a time
* only visible objects are rendered
* only relevant data is processed
...
### 10.8 Rendering Ownership

Rendering is owned by the active engine.

The frontend:

* provides the rendering surface
* manages interaction

The engine:

* determines what is rendered
* determines how objects behave

---

### 10.9 Hub Preview vs Engine Immersion
...
### 10.10 User Experience Goal

The user should feel:

* anchored in a real environment
* able to explore infinitely
* guided, not overwhelmed

The system should behave like:

> **A living model of reality that responds to interaction**
