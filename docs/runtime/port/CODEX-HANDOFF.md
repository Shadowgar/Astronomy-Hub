...
**Last updated:** 2026-04-27, evidence **EV-0122** (Stars Backend Slice S3 deeper CPU-side `render_finish`/`rend_flush` lifecycle modeling landed: explicit dispatch -> release -> flush-complete -> post-flush-reset records; direct star safety path preserved).
...
5. **`docs/runtime/port/evidence-index.md`** — the evidence ledger. Cite an existing `EV-xxxx` when referencing work; add a new row for any new work you land (**next free ID: EV-0123**; `EV-0067` and `EV-0068` are intentionally unused).
...
- When you land new work, append a new row to `evidence-index.md` with the next free ID (**next: EV-0123**). Use the existing column shape: `| Evidence ID | Module | Gate | Command / Probe | Artifact Path | Result | Notes |`.
