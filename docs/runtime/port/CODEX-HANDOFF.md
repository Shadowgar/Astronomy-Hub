...
**Last updated:** 2026-04-22, evidence **EV-0098** (`starsCRuntimePort.ts` lands a large runtime adapter seam for survey/list/lookup `stars.c` contracts; `starsLookup.ts` now routes through the adapter lookup seam; dedicated regression coverage in `test_module2_stars_c_runtime_port.test.js`; **119/119** / **21 files**; module1 **46/46**; typecheck + build PASS).
...
5. **`docs/runtime/port/evidence-index.md`** — the evidence ledger. Cite an existing `EV-xxxx` when referencing work; add a new row for any new work you land (**next free ID: EV-0099**; `EV-0067` and `EV-0068` are intentionally unused).
...
| `module2-stars-full` | **BLOCKED / partial** (active) | `npm run test:module2` → **119/119 / 21 files** | G0/G1 PASS (EV-0036). G2/G3/G4 partial (EV-0038–EV-0098). G5–G7 FAIL. See `module2-source-contract.md` §5 + §7. |
...
npm run test:module2     # 119/119 across 21 files (module 2 gate bundle as of EV-0098)
...
- When you land new work, append a new row to `evidence-index.md` with the next free ID (**next: EV-0099**). Use the existing column shape: `| Evidence ID | Module | Gate | Command / Probe | Artifact Path | Result | Notes |`.
