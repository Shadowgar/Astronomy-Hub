...
**Last updated:** 2026-04-26, evidence **EV-0111** (painter command queue slice: typed CPU-side command collection + finalize-on-`paint_finish`; batching/render_gl remains unported).
...
5. **`docs/runtime/port/evidence-index.md`** — the evidence ledger. Cite an existing `EV-xxxx` when referencing work; add a new row for any new work you land (**next free ID: EV-0114**; `EV-0067` and `EV-0068` are intentionally unused).
...
- When you land new work, append a new row to `evidence-index.md` with the next free ID (**next: EV-0114**). Use the existing column shape: `| Evidence ID | Module | Gate | Command / Probe | Artifact Path | Result | Notes |`.
