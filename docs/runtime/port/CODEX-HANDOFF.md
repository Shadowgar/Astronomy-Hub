...
**Last updated:** 2026-04-20, evidence **EV-0074** (module 2 native `render_visitor` traversal port; module 2 bundle 35/35 / 10 files; typecheck + module 0/1/2 bundles all PASS).
...
5. **`docs/runtime/port/evidence-index.md`** — the evidence ledger. Cite an existing `EV-xxxx` when referencing work; add a new row for any new work you land (**next free ID: EV-0075**; `EV-0067` and `EV-0068` are intentionally unused).
...
## 4. Module completion state (authoritative as of EV-0074)
...
| `module2-stars-full` | **BLOCKED / partial** (active) | `npm run test:module2` → **35/35 / 10 files** | G0/G1 PASS (EV-0036). G2/G3/G4 partial (EV-0038–EV-0074). G5–G7 FAIL. See `module2-source-contract.md` §5 + §7. |
...
1. **Survey-wide `obj_get_by_hip`.** The single-tile helper landed in EV-0044 (`adapters/starsLookup.ts::findRuntimeStarByHipInTiles`). Extend to a source-faithful survey traversal keyed off `hip_get_pix(hip, 2)` so selection lookups don't linearly scan every loaded tile.
2. **Extend G4 deterministic coverage** (`runtime/module2ParityFingerprint.ts`) to include the EV-0070 + EV-0074 projection slice now that traversal is landed.
...
npm run test:module2     # 35/35 across 10 files (module 2 gate bundle as of EV-0074)
...
| `obj_get_by_hip` (single-tile helper form) | `frontend/src/features/sky-engine/engine/sky/adapters/starsLookup.ts` |
...
- When you land new work, append a new row to `evidence-index.md` with the next free ID (**next: EV-0075**). Use the existing column shape: `| Evidence ID | Module | Gate | Command / Probe | Artifact Path | Result | Notes |`.
...
- "No stars visible / very few stars." Native `render_visitor` traversal is now landed (EV-0074), but survey-wide `obj_get_by_hip` traversal and deeper G4 coverage are still open; keep runtime symptom claims scoped to evidence-backed changes only.
