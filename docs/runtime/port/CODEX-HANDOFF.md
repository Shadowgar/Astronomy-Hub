...
**Last updated:** 2026-04-20, evidence **EV-0075** (module 2 survey-wide `obj_get_by_hip` lookup keyed by `hip_get_pix(hip, 2)`; module 2 bundle 38/38 / 11 files; typecheck + module 0/1/2 bundles all PASS).
...
5. **`docs/runtime/port/evidence-index.md`** — the evidence ledger. Cite an existing `EV-xxxx` when referencing work; add a new row for any new work you land (**next free ID: EV-0076**; `EV-0067` and `EV-0068` are intentionally unused).
...
## 4. Module completion state (authoritative as of EV-0075)
...
| `module2-stars-full` | **BLOCKED / partial** (active) | `npm run test:module2` → **38/38 / 11 files** | G0/G1 PASS (EV-0036). G2/G3/G4 partial (EV-0038–EV-0075). G5–G7 FAIL. See `module2-source-contract.md` §5 + §7. |
...
1. **Extend G4 deterministic coverage** (`runtime/module2ParityFingerprint.ts`) to include the EV-0070 + EV-0074 + EV-0075 projection/lookup slices now that traversal + survey lookup are landed.
2. **`stars_list` / `stars_add_data_source` seams.** Source-faithful datasource/list behavior in `stars.c` remains open after EV-0074/EV-0075.
...
npm run test:module2     # 38/38 across 11 files (module 2 gate bundle as of EV-0075)
...
- When you land new work, append a new row to `evidence-index.md` with the next free ID (**next: EV-0076**). Use the existing column shape: `| Evidence ID | Module | Gate | Command / Probe | Artifact Path | Result | Notes |`.
...
- "No stars visible / very few stars." Native `render_visitor` traversal (EV-0074) and survey-wide loaded-tile `obj_get_by_hip` lookup (EV-0075) are now landed; deeper G4 coverage and remaining `stars.c` list/datasource seams are still open, so keep runtime symptom claims scoped to evidence-backed changes only.
