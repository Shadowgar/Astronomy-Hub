...
**Last updated:** 2026-04-21, evidence **EV-0082** (G4 fingerprint adds deterministic `stars_list` slice; `docker:dev-backend` Track A helper; module 2 bundle 46/46 / 12 files; typecheck + module 1/2 bundles PASS).
...
5. **`docs/runtime/port/evidence-index.md`** — the evidence ledger. Cite an existing `EV-xxxx` when referencing work; add a new row for any new work you land (**next free ID: EV-0083**; `EV-0067` and `EV-0068` are intentionally unused).
...
| `module2-stars-full` | **BLOCKED / partial** (active) | `npm run test:module2` → **46/46 / 12 files** | G0/G1 PASS (EV-0036). G2/G3/G4 partial (EV-0038–EV-0082). G5–G7 FAIL. See `module2-source-contract.md` §5 + §7. |
...
npm run test:module2     # 46/46 across 12 files (module 2 gate bundle as of EV-0082)
...
- When you land new work, append a new row to `evidence-index.md` with the next free ID (**next: EV-0077**). Use the existing column shape: `| Evidence ID | Module | Gate | Command / Probe | Artifact Path | Result | Notes |`.
