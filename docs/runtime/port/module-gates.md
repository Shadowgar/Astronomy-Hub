...
**Module 2 (stars-full) — active; Overall BLOCKED:** **`module1-hips-kernel`** is **`COMPLETE`** (**EV-0034**). **`G0`/`G1`** promoted (**EV-0036**): four inventory rows + **`module2-source-contract.md`**. **`BLK-003`** is **RESOLVED** (**EV-0037**, **`stellarium-web-engine-src.md`**) — pinned GitHub `src/` paths for **`hip`/`stars`/`bv_to_rgb`**. **`G2`–`G7`** remain **FAIL** until implementation + evidence (not a missing-source blocker). Execution-order note: **EV-0035**.
