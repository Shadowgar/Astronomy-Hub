...
**Module 2 (stars-full) — active; Overall BLOCKED:** **`module1-hips-kernel`** is **`COMPLETE`** (**EV-0034**). **`G0`/`G1`** promoted (**EV-0036**). **`BLK-003`** **RESOLVED** (**EV-0037**). **`G2` partial:** **`bv_to_rgb`** (**EV-0038**); **`nuniq_to_pix`** (**EV-0039**); **`render_visitor` limit magnitude** (**EV-0040**) plus native tile traversal loop / tile gates / point-clipped path on the runtime surface (**EV-0074**); **`hip_get_pix` / vendored `hip.inl` table** (**EV-0041**), plus removal of local projected-star floor/cap heuristics and packet-time star/label heuristics on the active stars path (**EV-0063**, **EV-0065**, **EV-0066**); and ERFA **`eraStarpv`** + **`eraEpb2jd`** + Stellarium **`stars.c::compute_pv` / `star_get_astrom`** ports replacing the hand-rolled `raDec → ICRF` shortcut for catalogue stars with measurable parallax (**EV-0070**). **`G3` partial:** Hipparcos tile merge + HIP ↔ HEALPix consistency (**EV-0042**), `obj_get_by_hip` helper (**EV-0044**) + scene wiring / HIP detail route / selection continuity (**EV-0045**, **EV-0046**, **EV-0047**), survey-wide loaded-tile lookup now aligned with `obj_get_by_hip` order traversal (`hip_get_pix(hip, 0)` then `hip_get_pix(hip, 1)`) (**EV-0075**, **EV-0091**), loaded-runtime `stars_list` loop traversal corrected to source row-order/continue semantics (no local sort+break heuristic, **EV-0092**), hint traversal corrected to source no-`max_mag` filtering semantics (**EV-0093**), no-hint iterator control flow aligned with `hips_iter_next`/`hips_iter_push_children` semantics via `starsListTraversal.ts` (**EV-0094**), survey-registration ordering/gating centralized through the `starsSurveyRegistry.ts` parity adapter consumed by `fileTileRepository.ts` (**EV-0095**, comparator tie-scope parity **EV-0096**), and a large runtime seam extraction into `starsCRuntimePort.ts` for survey/list/lookup contracts consumed by `starsLookup.ts` (**EV-0098**); top/bottom HUD structure and asset parity passes (**EV-0049**, **EV-0050**, **EV-0053**) with intermediate dock experiments explicitly superseded (**EV-0051**, **EV-0052**), runtime tile-query signature stabilization against viewport float jitter to stop reload churn (**EV-0069**), scene assembler wiring of `star_get_astrom` through the observer-frame chain with per-star `pv` caching (**EV-0070**), and StarsModule consumption of tile traversal output via `starsRenderVisitor.ts` (**EV-0074**). **`G4` partial:** module 2 port algorithm fingerprint (**EV-0043**, extended **EV-0054**, **EV-0061**, **EV-0076**, **EV-0082**, **EV-0088**, **EV-0089**, **EV-0096**, **EV-0097**, **EV-0099**) now includes deterministic traversal order/entries, survey HIP lookup results, catalog-astrometry propagation slices, `stars_list` slices, scene-luminance slices, fixed-input runtime projection replay over `collectProjectedStars`, projection-cache reuse decision replay from `StarsModule` thresholds, survey-registry ordering tie behavior, overlay cadence decision behavior from the active runtime seam, and stars-c-runtime adapter outcomes from the EV-0098 seam; explicit module2 bundle regression suites for reuse thresholds/streak behavior (**EV-0090**), survey-registration/load-plan semantics (**EV-0095**), overlay cadence decisions (**EV-0097**), and stars.c runtime adapter list/lookup semantics (**EV-0098**) are now in-bundle. **`G5`–`G7`** **FAIL**. Cross-module regression sweep through module0/module1/module2 bundles is recorded in **EV-0062**. Full runtime-port doc audit reconciliation (module-inventory function tables for module 2, `test:module2` bundle expansion to include `test_erfa_starpv.test.js`, repo-wide residuals catalog below) is recorded in **EV-0071**. `painter_project(FRAME_ASTROM → FRAME_OBSERVED)` apparent-place chain on the EV-0070 pv output is now sealed end-to-end by regression test + `test:module2` bundle extension (**EV-0072**), traversal bundle extension landed in **EV-0074**, `obj_get_by_hip` survey lookup extension is tightened by EV-0091, `stars_list` loop/hint/iterator semantics are tightened by EV-0092/EV-0093/EV-0094, survey registration seams were extended by EV-0095/EV-0096, runtime adapter seam extraction landed in EV-0098, and deterministic replay sealing for that seam landed in EV-0099; G4 deterministic replay coverage has now been extended through **EV-0099**. `test:module2` is now **119/119 across 21 files**. Execution-order note: **EV-0035**. See **`module2-source-contract.md` §7** for agent handoff (files, commands, next steps).
...
G2/G3 `stars_list` hint alignment (2026-04-22, **EV-0093**): hinted traversal in `listRuntimeStarsFromTiles` now mirrors `stars.c::stars_list` by removing local `max_mag` filtering once `hint=nuniq` resolves to a tile. `test_module2_stars_list.test.js` adds a regression that locks no-`max_mag` hint behavior. `npm run typecheck`, `npm run build`, `npm run test:module2` (72/72), and `npm run test:module1` (46/46) PASS.

G2/G3 `stars_list` iterator traversal alignment (2026-04-22, **EV-0094**): added `starsListTraversal.ts` with explicit `StarsListTraversalCursor` and no-hint visitor loop to mirror `stars.c` iterator control flow (`next` + conditional `push_children`). `listRuntimeStarsFromTiles` now runs non-hint traversal through this port and `test_module2_stars_list_traversal.test.js` locks cursor, skip/push, mag gate, callback-break, and source-filter behavior. `npm run typecheck`, `npm run build`, `npm run test:module2` (79/79), and `npm run test:module1` (46/46) PASS.

G2/G3 `stars_add_data_source` survey registration alignment (2026-04-22, **EV-0095**): added `starsSurveyRegistry.ts` to port `survey_cmp` ordering semantics, Gaia `min_vmag` promotion, activation-floor derivation, narrow-FOV Gaia entry policy, and deterministic active-survey load-plan resolution; `fileTileRepository.ts` now consumes this shared adapter instead of local inline survey-order/gating logic. Added `test_module2_stars_survey_registry.test.js` and wired it into `test:module2` and module2 CI path filters. `npm run typecheck`, `npm run build`, `npm run test:module2` (96/96), and `npm run test:module1` (46/46) PASS.

G2/G3/G4 `survey_cmp` tie-scope alignment (2026-04-22, **EV-0096**): `compareStarsSurveyByMaxVmag` now mirrors `stars.c::survey_cmp` comparator scope exactly (max-vmag only; no Hub tie-breakers), `test_module2_stars_survey_registry.test.js` locks insertion-order tie behavior for equal `max_vmag`, and `computeModule2PortFingerprint` now includes a `survey-registry` deterministic slice with updated snapshot coverage. `npm run typecheck`, `npm run build`, `npm run test:module2` (96/96), and `npm run test:module1` (46/46) PASS.

G4 overlay cadence replay extension (2026-04-22, **EV-0097**): added `overlayCadence.ts::evaluateOverlayCadenceDecision` and wired `OverlayRuntimeModule.ts` to consume this pure cadence seam; `computeModule2PortFingerprint` now includes `overlay-cadence` deterministic outcomes and `test_module2_overlay_cadence.test.js` locks force-sync/view-change behavior (including azimuth wrap-around). `npm run typecheck`, `npm run build`, `npm run test:module2` (100/100), and `npm run test:module1` (46/46) PASS.

G4 stars runtime adapter replay extension (2026-04-22, **EV-0099**): `computeModule2PortFingerprint` now includes a deterministic `stars-c-runtime` slice that freezes EV-0098 adapter outcomes (survey ordering, source resolution fallback, list traversal statuses, and HIP lookup statuses) with snapshot + assertion updates in `test_module2_deterministic_replay.test.js`. `npm run typecheck`, `npm run build`, `npm run test:module2` (119/119), and `npm run test:module1` (46/46) PASS.
