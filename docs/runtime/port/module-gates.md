...
*Module 2: gate columns remain **`FAIL`** until each gate is fully **`PASS`**; partial Hub work is documented in **`module2-source-contract.md` §5** and **`evidence-index.md`** (**EV-0038**–**EV-0043**).*
...
**Module 2 (stars-full) — active; Overall BLOCKED:** **`module1-hips-kernel`** is **`COMPLETE`** (**EV-0034**). **`G0`/`G1`** promoted (**EV-0036**). **`BLK-003`** **RESOLVED** (**EV-0037**). **`G2` partial:** **`bv_to_rgb`** (**EV-0038**); **`nuniq_to_pix`** (**EV-0039**); **`render_visitor` limit magnitude** (**EV-0040**); **`hip_get_pix` / `hip.inl`** (**EV-0041**). **`G3` partial:** Hipparcos tile merge + HIP ↔ HEALPix consistency (**EV-0042**), `obj_get_by_hip`-style lookup helper (**EV-0044**), live scene runtime wiring (**EV-0045**), stable HIP detail route identity (**EV-0046**), selection continuity with id-resync (**EV-0047**), night-visible startup + Stellarium HUD alignment pass (**EV-0048**), Stellarium-like control-strip grouping (**EV-0049**), icon-first compact control chrome pass (**EV-0050**), left tool-dock geometry pass (**EV-0051**), right time-dock + simplified bottom strip zoning (**EV-0052**), and vendored Stellarium simple-html toolbar SVGs + layout refinements (**EV-0053**). **`G4` partial:** module 2 port algorithm fingerprint (**EV-0043**, extended **EV-0054**). **`G5`–`G7`** **FAIL**. Execution-order note: **EV-0035**. See **`module2-source-contract.md` §7** for agent handoff (files, commands, next steps).
