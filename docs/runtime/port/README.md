...
- `evidence-index.md`: evidence artifacts and command/output references. **Next free ID: EV-0105** (EV-0067 / EV-0068 are intentionally unused).
...
**`module0-foundation-lock`** is **`COMPLETE`** (**EV-0019**). **`module1-hips-kernel`** is **`COMPLETE`** (**EV-0034**): **`eph-file.c`** / HEALPix / tile / HiPS-order spine, deterministic tile-load replay (**EV-0024**), normalized **`hipsViewport`** + quadtree depth from **`hips_get_render_order`** (**EV-0032**–**EV-0033**), and full evidence closure. **`module2-stars-full`** is the active execution-order target (**EV-0035**); **`G0`/`G1`** **PASS** (**EV-0036**). Hub subset progress includes stars pipeline parity, UI structure/asset parity hardening, runtime stabilization + cleanup, the ERFA `eraStarpv` / Stellarium `compute_pv` / `star_get_astrom` catalogue astrometry port wired through the scene assembler with a per-star pv cache (**EV-0038**–**EV-0070**), the full runtime-port doc audit + `test:module2` bundle expansion to include `test_erfa_starpv.test.js` (**EV-0071**), the end-to-end `painter_project(FRAME_ASTROM → FRAME_OBSERVED)` regression that seals the EV-0070 pv output through `eraLdsun` + `eraAb` + `bpn^T` + `ri2h` (**EV-0072**), native `stars.c::render_visitor` tile traversal semantics wired onto the active StarsModule/runtimeFrame path (**EV-0074**), source-faithful loaded-tile `obj_get_by_hip` order traversal (`hip_get_pix(hip, 0)` then `hip_get_pix(hip, 1)`) in `starsLookup.ts` (**EV-0091**, superseding the earlier order-2 bucket approximation from **EV-0075**), source-faithful `stars_list` traversal semantics for non-hint and hint paths with explicit iterator descent behavior in `starsList.ts` + `starsListTraversal.ts` (**EV-0092**, **EV-0093**, **EV-0094**), a dedicated `stars_add_data_source` survey-registration parity seam in `starsSurveyRegistry.ts` consumed by `fileTileRepository.ts` load planning (**EV-0095**, comparator tie semantics tightened in **EV-0096**), a new large `stars.c` runtime adapter seam in `starsCRuntimePort.ts` (survey/list/lookup contracts) consumed by `starsLookup.ts` with dedicated parity regressions (**EV-0098**), deterministic replay coverage for this adapter seam through a new `stars-c-runtime` fingerprint slice in `module2ParityFingerprint.ts` (**EV-0099**), a new large `stars.c` survey lifecycle seam in `starsCSurveyLifecyclePort.ts` (**EV-0100**), a new large `stars.c` module-runtime seam in `starsCModuleRuntimePort.ts` with deterministic replay `stars-c-module-runtime` coverage (**EV-0101**), the large `framePacingDecisions.ts` deterministic trace seam for projection reuse / overlay cadence / runtime model sync (**EV-0102**), the large `starsCLabelRuntimePort.ts` label/name/designation seam for `stars.c::star_render_name` adjacent behavior (**EV-0103**), and the initial side-by-side parity harness baseline in `module2SideBySideParityHarness.ts` with dedicated bundle/CI coverage (**EV-0104**). Deterministic replay coverage includes traversal/lookup/astrometry, `stars_list`, scene luminance, fixed-input `collectProjectedStars` replay, projection-cache reuse decision replay, survey-registry ordering parity slice, overlay runtime cadence decisions, stars-c-runtime adapter decisions, stars-c-survey-lifecycle decisions, stars-c-module-runtime decisions, stars-c-frame-pacing trace decisions, and stars-c-label-runtime decisions in the module2 bundle (**EV-0076**, **EV-0082**, **EV-0086**, **EV-0088**, **EV-0089**, **EV-0090**, **EV-0096**, **EV-0097**, **EV-0099**, **EV-0100**, **EV-0101**, **EV-0102**, **EV-0103**, **EV-0104**; `test:module2` currently 171/171 across 26 files). **Overall** **`BLOCKED`** until remaining gates close; see **`module2-source-contract.md` §5–§7**, **`module-gates.md`** "Known residuals", and **`evidence-index.md`**.
