...
| EV-0020 | module1-hips-kernel | G0–G7 kickoff | `npm run typecheck && npm run build && npm run test:module1` (from `frontend/`; **`test:module1`** = `vitest run` on `test_file_backed_tile_repository`, `test_eph_codec`, `test_sky_engine_runtime_slice`) | `module1-source-contract.md`, `module-gates.md` (module 1 row), `frontend/package.json` (`test:module1`) | PASS | Promotes **`module1-hips-kernel`** from all-**`N/A`**; **G0/G1/G6** recorded **PASS**; **G2–G5/G7** **FAIL** until parity and evidence closure; **`test_close_fov_star_counts.test.js`** excluded (long-run / asset-heavy). |
