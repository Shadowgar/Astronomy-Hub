...
| Scene diagnostics + `stars_list` live probe | `frontend/src/features/sky-engine/engine/sky/diagnostics/skyDiagnostics.ts` (`buildSkyDiagnostics` → `listRuntimeStarsFromTiles`), `contracts/scene.ts` (`diagnostics.starsListVisitCount`) (**EV-0083**) |
