...
- **G4 (Hub):** `runtime/module2ParityFingerprint.ts` + `tests/test_module2_deterministic_replay.test.js` (snapshot; Evidence **EV-0043** / extended **EV-0054** / **EV-0061** / **EV-0076** / **EV-0082** / **EV-0088**). Fingerprint now covers the prior algorithm set (`bv_to_rgb`, `nuniq_to_pix`, `resolveStarsRenderLimitMagnitude`, `hip_get_pix`, `coreGetPointForMagnitude`, view-tier / label-cap policy, `syncCadenceMs`, FOV `starCap`) plus deterministic slices for `stars.c::render_visitor` traversal order + yielded stars (`visitStarsRenderTiles`), survey-wide `obj_get_by_hip` lookup bucket semantics (`findRuntimeStarByHipInTiles`), `compute_pv`/`star_get_astrom` catalog astrometry propagation invariants, `stars_list` loaded-tile traversal + hint/`again` statuses (`listRuntimeStarsFromTiles`), and fixed-input runtime projection replay over `collectProjectedStars` (`stars-projection`). Live scene packets additionally carry **`starsListVisitCount`** from `skyDiagnostics.ts` (**EV-0083**).
