...
- **`eraApcs` / `eraApco`:** **`eraApcs`** **PORTED** (`erfaApcs.ts`, **EV-0015**); **`eraApco`** **PORTED** (`erfaApco.ts`, SOFA vector test, **EV-0016**). **`deriveObserverGeometry`** fills **`SkyObserverDerivedGeometry.astrom`**. **`eraAb` / `eraLdsun`** **PORTED** (`erfaAbLdsun.ts`, **`test_erfa_ab_ldsun.test.js`**); wiring **`stellariumAstrometricToApparentIcrsUnit`** into **`convertObserverFrameVector`** / **`frames.c`** parity still **BLOCKED** (`module0-eraApco-port-plan.md`).
