...
| `src/modules/stars.c` | module | module2-stars-full | BLOCKED | Hub TS (partial ports): `bvToRgb.ts` (**EV-0038**), `starsNuniq.ts` (**EV-0039**, `nuniq_to_pix`), `stellariumPainterLimits.ts` (**EV-0040**, `render_visitor` `limit_mag` policy), `runtime/starsRenderVisitor.ts` + `runtime/modules/StarsModule.ts` + `runtime/modules/runtimeFrame.ts` (**EV-0074**, native `render_visitor` tile traversal + point-clipped gate on active runtime path; **EV-0087**, non-star horizon fade uses geometric altitude only + horizon/projection Vitests), `starsLookup.ts` (**EV-0075**, survey-wide `obj_get_by_hip` lookup; **EV-0091**, aligns traversal to `hip_get_pix(hip, 0)` then `hip_get_pix(hip, 1)`), `starsListTraversal.ts` (**EV-0094**, explicit `stars_list` no-hint iterator cursor + conditional child descent parity), `erfaStarpv.ts` + `starsCatalogAstrom.ts` (**EV-0070**, `compute_pv` + `star_get_astrom` + ERFA `eraStarpv` / `eraEpb2jd`), `services/sceneAssembler.ts` (**EV-0070**/**EV-0074**, per-star pv cache + tile-ordered packet feed), `contracts/scene.ts` (tile traversal payload schema), and `adapters/starsSurveyRegistry.ts` (**EV-0095**, `stars_add_data_source` ordering/gating/load-plan seam; **EV-0096**, `survey_cmp` comparator tie-scope parity: max-vmag-only comparison with insertion-order ties). `painter_project(FRAME_ASTROM → FRAME_OBSERVED)` apparent-place chain (`eraLdsun` + `eraAb` + `bpn^T` + `ri2h`) is **PORTED** via `erfaAbLdsun.ts` + `convertObserverFrameVector('icrf' → 'observed_geom')` and sealed end-to-end by **EV-0072** (`test_module2_painter_project_frame_astrom.test.js`); deterministic replay now also locks fixed-input runtime `collectProjectedStars` projection output (**EV-0088**), `StarsModule` projection-cache reuse decisions via `evaluateStarsProjectionReuse` (**EV-0089**), and survey-registry ordering/comparator tie behavior via `module2ParityFingerprint.ts` snapshot coverage (**EV-0096**), with explicit module2-bundle regression coverage in `test_module2_stars_projection_reuse.test.js` (**EV-0090**). Native C `stars_list` and `stars_add_data_source` remain **BLOCKED**. **Gate:** G5. See **`module2-source-contract.md`** §1–§7. |
...
- module2-stars-full: **`G0`/`G1`** **PASS** (**EV-0036**); **`BLK-003`** **RESOLVED** (**EV-0037**); partial-parity Hub ports landed through **EV-0038**–**EV-0096** (`bv_to_rgb`, `nuniq_to_pix`, `render_visitor` `limit_mag`, `hip_get_pix` + vendored `PIX_ORDER_2` table, Hipparcos tile merge HIP-check, survey-wide `obj_get_by_hip` lookup with source-order traversal (`order 0` then `order 1`), source-faithful loaded-runtime `stars_list` non-hint row traversal (`continue` over over-limit rows without per-tile sort/break), source-faithful hinted `stars_list` traversal (no local `max_mag` filtering), explicit iterator/child-descent `stars_list` traversal parity (`starsListTraversal.ts`), dedicated `stars_add_data_source` survey-registration adapter parity (`starsSurveyRegistry.ts`) with `survey_cmp` tie-scope correction, stable HIP detail route + selection continuity, deterministic algorithm fingerprint, ERFA `eraStarpv` / `eraEpb2jd` + Stellarium `compute_pv` / `star_get_astrom`, native `render_visitor` tile traversal on runtime path, and **`painter_project(FRAME_ASTROM → FRAME_OBSERVED)`** apparent-place chain sealed end-to-end). Module 2 function inventory table below; see **`module2-source-contract.md`** §2–§7.
...
- **G4 (Hub):** `runtime/module2ParityFingerprint.ts` + `tests/test_module2_deterministic_replay.test.js` + `tests/test_module2_stars_projection_reuse.test.js` (Evidence **EV-0043** / extended **EV-0054** / **EV-0061** / **EV-0076** / **EV-0082** / **EV-0088** / **EV-0089** / **EV-0090**). Fingerprint now covers the prior algorithm set (`bv_to_rgb`, `nuniq_to_pix`, `resolveStarsRenderLimitMagnitude`, `hip_get_pix`, `coreGetPointForMagnitude`, view-tier / label-cap policy, `syncCadenceMs`, FOV `starCap`) plus deterministic slices for `stars.c::render_visitor` traversal order + yielded stars (`visitStarsRenderTiles`), survey-wide `obj_get_by_hip` lookup semantics (`findRuntimeStarByHipInTiles`; runtime traversal alignment updated in **EV-0091**), `compute_pv`/`star_get_astrom` catalog astrometry propagation invariants, `stars_list` loaded-tile traversal + hint/`again` statuses (`listRuntimeStarsFromTiles`), fixed-input runtime projection replay over `collectProjectedStars` (`stars-projection`), and projection-cache reuse decision replay over `evaluateStarsProjectionReuse` (`stars-reuse`); dedicated regression assertions now lock threshold/streak behavior independently of snapshot shape. Live scene packets additionally carry **`starsListVisitCount`** from `skyDiagnostics.ts` (**EV-0083**).
