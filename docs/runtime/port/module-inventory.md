...
- **`eraApcs` / `eraApco`:** **`eraApcs`** **PORTED** (`erfaApcs.ts`, **EV-0015**); **`eraApco`** **PORTED** (`erfaApco.ts`, SOFA vector test, **EV-0016**). **`deriveObserverGeometry`** fills **`SkyObserverDerivedGeometry.astrom`**. **`eraAb` / `eraLdsun`** + **`convertObserverFrameVector`** Stellarium CIO path (`erfaAbLdsun.ts`, **`coordinates.ts`**, **`StarsModule` / `ObjectRuntimeModule`**); catalog / scene vectors still mostly classical unless callers use the new path (`module0-eraApco-port-plan.md`).
