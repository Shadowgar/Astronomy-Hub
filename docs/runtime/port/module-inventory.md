...
| `src/obj.h` | core | module7-remaining-swe | BLOCKED | `runtime/StellariumObjectRuntime.ts` now carries the first source-shaped `obj_t` / `obj_klass_t` subset for class registration/lookup, create/create_str, retain/release, clone, render dispatch, designation-name selection, immutable JSON data, attribute lookup/call helpers, and first `obj_get_info` typed-info fallbacks (**EV-0132**). Full native member-offset attribute bridge, frame-backed `obj_get_pos`/`INFO_LHA`, object paths, JSON tree dumping, and module/core integration remain blocked. |
...
| `src/core.c` | core | module0-foundation-lock | BLOCKED | Barometric + time/refraction hooks: `transforms/coordinates.ts` (`computeStellariumBarometricPressureMbar`), `SkyClockService`, bridge. Frame lifecycle port pivot landed in `runtime/SkyCore.ts` (`runFrameLifecycle`) with source-faithful phase order from `core_update`/`core_render` (`core.c` lines 285-347, 521-598), plus minimal execution-order regression `tests/test_sky_core_frame_lifecycle_order.test.js` and mapping artifact `docs/runtime/port/core-c-frame-lifecycle-mapping.md` (**EV-0107**). `runtime/StellariumCoreRuntime.ts` now owns a source-shaped first function group for `core_init`, `core_release`, `core_update`, `core_render`, `core_add_task`, `core_get_proj`, `core_get_point_for_mag` handoff through painter-limit computation, and `core_report_luminance_in_fov`, with focused coverage in `tests/test_stellarium_core_runtime.test.js` (**EV-0130**). Full `core_t` object tree, input/search/selection, native observer/projection immutability asserts, and `painter.c` / `render_gl.c` internals remain unported. **Gate:** G5. |
...
| `src/projection.h` | projection | module0-foundation-lock | BLOCKED | `runtime/StellariumProjectionRuntime.ts` now carries the generic `projection_t` / `projection_klass_t` constants and source-shaped class registry, FOV delegation, projection init, clip/window projection, and unproject matrix wrapper (**EV-0133**). `projectionMath.ts` / `SkyProjectionService.ts` remain the active runtime surface. Native `projections/*.c` class implementations and active-service integration remain blocked. **Gate:** G5. |
...
| `src/core.h` | core | module0-foundation-lock | BLOCKED | Types/contracts spread across `sky-engine` runtime + feature modules; `runtime/StellariumCoreRuntime.ts` now carries the first source-shaped `task_t`/core-state subset and exported source-function anchor list (**EV-0130**). No complete 1:1 `core_t` header port yet. **Gate:** G5. |
...
| `src/module.h` | module_system | module7-remaining-swe | BLOCKED | `runtime/StellariumModuleRuntime.ts` now carries the first source-shaped `MODULE_AGAIN` / module API subset and exported source-function anchor list (**EV-0131**). Full `MODULE_ITER`, JSON tree/path helpers, and `obj.c` class-registry integration remain blocked. |
...
| `src/projections/proj_hammer.c` | projection | module0-foundation-lock | BLOCKED | `runtime/StellariumProjectionRuntime.ts` now includes `createStellariumHammerProjectionKlass` with source-shaped `proj_hammer_project`, `proj_hammer_backward`, and `proj_hammer_init` behavior plus focused coverage in `tests/test_stellarium_projection_runtime.test.js` (**EV-0134**). Active projection registry/service integration remains blocked. **Gate:** G5. |
...
| `src/projections/proj_mercator.c` | projection | module0-foundation-lock | BLOCKED | `runtime/StellariumProjectionRuntime.ts` now includes `createStellariumMercatorProjectionKlass` with source-shaped `proj_mercator_project`, `proj_mercator_backward`, and `proj_mercator_init` behavior plus focused coverage in `tests/test_stellarium_projection_runtime.test.js` (**EV-0135**). Active projection registry/service integration remains blocked. **Gate:** G5. |
...
| `src/obj.c` | object | module7-remaining-swe | BLOCKED | `runtime/StellariumObjectRuntime.ts` now owns a source-shaped first function group for `obj_register_`, `obj_get_all_klasses`, `obj_get_klass_by_name`, `obj_create`, `obj_create_str`, `obj_retain`, `obj_release`, `obj_clone`, `obj_render`, `obj_get_name`, `obj_get_id`, `obj_get_designations`, `obj_get_json_data`, `obj_get_json_data_str`, `obj_get_attr_`, `obj_foreach_attr`, `obj_has_attr`, `obj_call_json`, `obj_call_json_str`, `obj_get_pvo`, and `obj_get_info` fallbacks for `INFO_RADEC`, `INFO_DISTANCE`, and `INFO_SEARCH_VMAG`, with focused coverage in `tests/test_stellarium_object_runtime.test.js` (**EV-0132**). Full native member-offset attribute bridge, variadic `obj_get_attr`/`obj_set_attr`, frame-backed `obj_get_pos` / `INFO_LHA`, object paths, and JSON tree dumping remain blocked. |
...
| `src/module.c` | module_system | module7-remaining-swe | BLOCKED | `runtime/StellariumModuleRuntime.ts` now owns a source-shaped first function group for `module_update`, `module_list_objs`, `module_list_objs2`, `module_add_data_source` retry scheduling through `core_add_task`, `module_get_render_order`, `module_add_global_listener`, `module_changed`, `module_add`, `module_add_new`, `module_remove`, and `module_get_child`, with focused coverage in `tests/test_stellarium_module_runtime.test.js` (**EV-0131**). Full JSON tree/path helpers and full `obj.c` object registry integration remain blocked. |
...
| `src/projection.c` | projection | module0-foundation-lock | BLOCKED | `runtime/StellariumProjectionRuntime.ts` now owns the first generic `projection.c` function group for `proj_register_`, `projection_compute_fovs`, `projection_init`, `project_to_clip`, `project_to_win`, `project_to_win_xy`, and `unproject`, with focused coverage in `tests/test_stellarium_projection_runtime.test.js` (**EV-0133**). Active service integration and native projection class implementations remain blocked. **Gate:** G5. |
