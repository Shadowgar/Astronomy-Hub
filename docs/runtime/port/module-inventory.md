...
| `stars_list` / `stars_add_data_source` | modules/stars.c | BLOCKED | `render_visitor` runtime traversal is ported (**EV-0074**) and survey-wide loaded-tile `obj_get_by_hip` lookup is ported (**EV-0075**). `fileTileRepository.ts` now mirrors `survey_cmp` ordering semantics (`NaN max_vmag` sorted last) and Gaia `min_vmag` promotion over non-Gaia survey ceilings while preserving narrow-FOV Gaia activation behavior (**EV-0077**). Remaining open seams are full `stars_list` source/hint traversal parity and side-by-side closure. **Gate:** G5 (see `module2-source-contract.md` §7 "Suggested next coding targets"). |
