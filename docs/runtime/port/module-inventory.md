...
- module2-stars-full: **`G0`/`G1`** **PASS** (**EV-0036**); **`BLK-003`** **RESOLVED** (**EV-0037**); partial-parity Hub ports landed through **EV-0038**–**EV-0074** (`bv_to_rgb`, `nuniq_to_pix`, `render_visitor` `limit_mag`, `hip_get_pix` + vendored `PIX_ORDER_2` table, Hipparcos tile merge HIP-check, `obj_get_by_hip`-style lookup, stable HIP detail route + selection continuity, deterministic algorithm fingerprint, ERFA `eraStarpv` / `eraEpb2jd` + Stellarium `compute_pv` / `star_get_astrom`, native `render_visitor` tile traversal on runtime path, and **`painter_project(FRAME_ASTROM → FRAME_OBSERVED)`** apparent-place chain sealed end-to-end). Module 2 function inventory table below; see **`module2-source-contract.md`** §2–§7.
...
| `obj_get_by_hip` (single-tile helper form) | modules/stars.c | PORTED (partial) | `frontend/src/features/sky-engine/engine/sky/adapters/starsLookup.ts` (`findRuntimeStarByHipInTiles`, `buildHipDetailRoute`, `resolveHipDetailRouteForRuntimeStar`); wired into `SkyEngineScene.tsx`. Tests: `test_module2_stars_lookup.test.js`, `test_sky_engine_selection_detail_route.test.js`. Evidence **EV-0044**, **EV-0045**, **EV-0046**, **EV-0047**. Full survey-wide `obj_get_by_hip` traversal still **BLOCKED**. |
| `stars_list` / `stars_add_data_source` / survey-wide `obj_get_by_hip` traversal | modules/stars.c | BLOCKED | `render_visitor` runtime traversal is now ported (**EV-0074**) and consumes source-aligned gates; remaining open work is survey-wide `obj_get_by_hip` and datasource/list seams. **Gate:** G5 (see `module2-source-contract.md` §7 "Suggested next coding targets"). |
...
- **G4 (Hub):** `runtime/module2ParityFingerprint.ts` + `tests/test_module2_deterministic_replay.test.js` (snapshot; Evidence **EV-0043** / extended **EV-0054** / **EV-0061**). Fingerprint covers `bv_to_rgb`, `nuniq_to_pix`, `resolveStarsRenderLimitMagnitude`, `hip_get_pix` samples, `coreGetPointForMagnitude` + view-tier / label-cap policy, and active runtime perf knobs (`syncCadenceMs`, FOV `starCap` samples).
