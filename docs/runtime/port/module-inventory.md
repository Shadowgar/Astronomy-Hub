...
- module2-stars-full: **`G0`/`G1`** **PASS** (**EV-0036**); **`BLK-003`** **RESOLVED** (**EV-0037**); partial-parity Hub ports landed through **EV-0038**–**EV-0101** (`bv_to_rgb`, `nuniq_to_pix`, `render_visitor` `limit_mag`, `hip_get_pix` + vendored `PIX_ORDER_2` table, Hipparcos tile merge HIP-check, survey-wide `obj_get_by_hip` lookup with source-order traversal (`order 0` then `order 1`), source-faithful loaded-runtime `stars_list` non-hint row traversal (`continue` over over-limit rows without per-tile sort/break), source-faithful hinted `stars_list` traversal (no local `max_mag` filtering), explicit iterator/child-descent `stars_list` traversal parity (`starsListTraversal.ts`), dedicated `stars_add_data_source` survey-registration adapter parity (`starsSurveyRegistry.ts`) with `survey_cmp` tie-scope correction, overlay cadence decision seam extraction (`overlayCadence.ts`) consumed by runtime overlay sync, large `starsCRuntimePort.ts` adapter seam, large `starsCSurveyLifecyclePort.ts` lifecycle seam, large `starsCModuleRuntimePort.ts` module-runtime seam, deterministic replay sealing for stars-c-runtime, stars-c-survey-lifecycle, and stars-c-module-runtime outcomes, stable HIP detail route + selection continuity, deterministic algorithm fingerprint, ERFA `eraStarpv` / `eraEpb2jd` + Stellarium `compute_pv` / `star_get_astrom`, native `render_visitor` tile traversal on runtime path, and **`painter_project(FRAME_ASTROM → FRAME_OBSERVED)`** apparent-place chain sealed end-to-end). Module 2 function inventory table below; see **`module2-source-contract.md`** §2–§7.
