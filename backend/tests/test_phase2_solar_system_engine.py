...
def test_jpl_ephemeris_fetches_expected_body_set(monkeypatch):
    ...
    expected_ids = {"sun", "moon", "mercury", "venus", "mars", "jupiter", "saturn", "uranus", "neptune"}
...
def test_jpl_ephemeris_parses_horizons_ra_dec_from_observer_quantities(monkeypatch):
    ...
    def _http_get_json(url, *, params=None, timeout_s=5.0):
        ...
        return {
            "result": (
                "$$SOE\n"
                "2026-Jun-04 02:00 N   02 39 09.40 +14 52 48.0  340.700916 -31.540467 1.1 2.2 3.3\n"
                "$$EOE"
            )
        }
    ...
    assert first["source"] == "jpl_ephemeris"
