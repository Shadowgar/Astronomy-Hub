...
def test_deep_sky_scope_falls_back_to_lookup_when_phase1_scene_is_starved(monkeypatch):
    monkeypatch.setattr(
        scene_service,
        "build_phase1_scene_state",
        lambda parsed_location=None, as_of=None: {
    ...
    objects = payload.get("objects") or []
    assert len(objects) == 1
    assert objects[0].get("id") == "m42"
    assert objects[0].get("type") == "deep_sky"
    assert objects[0].get("provider_source") == "messier_catalog"
