...
| dso | `src/modules/dso.c` | `astronomy.ts`, `engine/sky/runtime/modules/DsoRuntimeModule.ts`, `DsoRenderer.ts`, `dsoVisuals.ts` | partially ported | The active deep-sky path now carries bounded morphology classes and apparent-size metadata, partitions DSO objects out of generic rendering, and syncs them through a dedicated DSO runtime module and renderer. | Catalog breadth, Stellarium-grade symbol coverage, orientation handling, and deeper filtering remain incomplete. | Preserve the dedicated DSO path and add bounded symbol breadth and orientation support before widening the catalog. |
